...
def main():
    ...
    for lang_source in langs:
        y_source = (y[lang_source] == _role)

        # transfer learning
        ## feature transformation via SVD
        k = 10
        U_s, s_s, V_s = linalg.svds(X[lang_source], k=k);

        T = np.dot(V_s.T, np.diag(1 / s_s))
        X_s = np.nan_to_num(np.dot(X[lang_source], T))

        ## classifier
        clf = RandomForestClassifier(n_estimators=n_trees, random_state=42)
        clf.fit(X_s, y_source)

        ## evaluation
        for lang_target in langs:
            y_target = (y[lang_target] == _role)
            if (len(np.unique(y_source)) == 1) or (len(np.unique(y_target)) == 1):   # ROC not defined
                auc = np.nan
            else:
                # feature transformation
                X_t = np.nan_to_num(np.dot(X[lang_target], T))
                y_predict = clf.predict_proba(X_t)[:,1]
