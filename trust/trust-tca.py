...
def main(f_source_graph):
    ...
    # feature transformation via TCA
    X_s, _, y_s, _ = train_test_split(X_s, y_s, test_size=0.9, random_state=4242)
    X_t, _, y_t, _ = train_test_split(X_t, y_t, test_size=0.9, random_state=4242)

    my_tca = tca.TCA(dim=30,kerneltype='rbf', kernelparam=1, mu=1)
    X_s, X_t, _ = my_tca.fit_transform(X_s, X_t)
